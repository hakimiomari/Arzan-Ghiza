...
const baseUrl = "http://localhost:8000/api/admin/sales";
...
const getAllOrders = async (url) => {
  await axios
    ...
    .then((res) => {
      allData.value = res.data.order.data;
...
const getChartData = async () => {
  await axios
    .get("http://localhost:8000/api/admin/chart", {
      headers: { Authorization: `Bearer ${user.token}` },
    })
    .then((res) => {
...
const createBarChart = () => {
  ...
  // doughnut
  console.log("bar");
  console.log(orderCountsByMonth.value.July);
