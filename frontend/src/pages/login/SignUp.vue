<template>
  <div
    class="row template"
    style="width: 100vw; align-items: center; justify-content: center"
  >
...
const register = async () => {
  ...
  if (!value.value) {
    ...
    try {
      await axios
        .post(
          "http://127.0.0.1:8000/api/bussiness/register",
          {
            name: data.value.name,
            email: data.value.email,
            phone: data.value.phone,
            ...
            district: data.value.district,
            valige: data.value.valige,
            bussiness_name: data.value.bussiness_name,
